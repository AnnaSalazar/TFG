...
descriptiva2 <- function(X, nom, dades){
  if (!(is.numeric(X) || class(X) == "Date")){ 
    frecs <- table(X, useNA = "no"); proportions <- frecs/n
    dataf <- data.frame(x = levels(X), y = frecs)
    print(ggplot(dataf, aes(x = levels(X), y = frecs)) + geom_bar(stat = "identity", 
          color = "#1d91c0", fill = "#1d91c0") + 
            scale_y_continuous(expand = c(0,0)) +
  scale_x_discrete(limits = lev) +  theme(
        axis.text.x=element_text(angle = 90, size = 12),
        axis.text.y=element_text(hjust = 1, vjust = 0.8, size = 15)) + theme_minimal())
   }else{
      print(ggplot(dades, aes(x = X)) + geom_bar(col = "#1d91c0", fill = 
      "#1d91c0") + theme_minimal())
       a <- descr(X, transpose = TRUE, stats = c("N.Valid", "min", "q1", "med", 
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Hora de l'accident"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
par(mfrow=c(1,2))
...
```{r echo=FALSE, out.width="50%", fig.align = 'center', comment = "", fig.cap=c("Anàlisi descriptiu de la variable Minut de l'accident"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Nombre de ferits a l’accident"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
par(mfrow=c(1,2))
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Nombre de conductors beguts"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
par(mfrow=c(1,2))
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Nombre de persones implicades en l'accident"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
par(mfrow=c(1,2))
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Edat"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
par(mfrow=c(1,2))
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Nombre de vehicles involucrats en l'accident"), fig.subcap=c("Histograma", "Boxplot"),  results='asis', warning=FALSE}
par(mfrow=c(1,2))
...
```{r echo=FALSE, out.width="50%", comment = "", fig.cap=c("Anàlisi descriptiu de la variable Velocitat estimada"), fig.subcap=c("Histograma", "Boxplot"), results='asis', warning=FALSE}
#fig.align = 'center',
par(mfrow=c(1,2))
...
```{r, out.width="50%", fig.cap= "Anàlisi descriptiu de la variable Dia", fig.subcap=c("Bar plot"), results='asis'}
