...
A més de gràfiques, es poden decorar les pàgines per fer-les més boniques, o per afegir informació. També és possible assignar un nom a la pàgina, i crear-ne tantes com siguin necessàries per al nostre informe. Així mateix, podem establir un nom per a tot l'informe, de manera que a l'hora de guardar-lo i compartir-lo sigui més fàcilment interpretable.
...
Per acabar, podem compartir el nostre informe perquè altres usuaris puguin tenir accés. Això es pot fer des de la pantalla d'inici de Looker Studio, seleccionant l'opció de compartir l'informe (Figura ~\ref{fig:ds6}) i afegint les adreces de correu dels qui vulguem fer lectors de l'informe (aquells qui poden veure l'informe, però no tenen permisos d'edició) o bé editors (poden veure i editar l'informe). 
...
Un cop hem formulat la nostra \emph{query}, a través de la funció \texttt{dbGetQuery()}, ens comunicarem amb l'API de Google. A l'esquerra s'hi troba l'esquema de la comunicació, mentre que a la dreta està la funció utlitzada.
...
Per portar a terme la sessió pràctica es va fer ús del conjunt de dades públic prèviament esmentat: \emph{Catalonia Cell Coverage}, que conté informació sobré l'estat de cobertura de la telefonia mòbil que es va recollir en el període dels anys des del 2015 fins al 2017.
...
\item Creeu una taula que reculli les ciutats i la xarxa dels 10 primers usuaris que es van quedar sense servei mòbil, obviant els casos en què el nom de la ciutat o de la xarxa sigui \textit{NULL}.
...
A continuació es farà una recopilació dels resultats obtinguts, d'on es pot veure un resum gràfic a la figura \ref{fig:val}.
...
La base de dades que serà utilitzada al llarg de l'estudi prové de l'agència estatal de trànsit dels Estats Units i conté tres taules, entre les quals es troba un llistat d’accidents de trànsit ocorreguts el desembre de 2015 als Estats Units, juntament amb un recompte de totes les persones (conductors, passatgers o vianants) involucrades als accidents i, finalment, un inventari de tots els vehicles també involucrats. 
...
Per fer aquest anàlisi s'usarà la taula d'accidents depurada, que té un total de 7 variables numèriques. Per tant, l'anàlisi de components principals tindrà un total de 7 components. Després de realitzar els càlculs corresponents, obtenim l'ACP de les variables numèriques a la figura \ref{fig:ACP1}.
...
Un canvi en una variable d'una unitat augmenta el \textit{log-odds} tant com el valor del seu pes assignat.
