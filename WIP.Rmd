...
x <- c("bigrquery", "kableExtra", "DBI", "knitr", "dplyr", "VIM", "tidyverse", "summarytools", "reshape2", "ggplot2", "devtools", "naniar", "forcats", "sqldf")
...
La nostra base de dades depurada té un total de 6 variables numèriques. Per tant, l'anàlisi de components principals tindrà un total de 6 components. Després de realitzar els càlculs corresponents, obtenim que l'ACP de les variables numèriques és el següent:
...
```{r, echo=FALSE, out.width="50%", fig.align='center', fig.cap="Barplot de la inèrcia de cada component"}
barplot(pinerEix, 
        main="Percentatge del total d'inèrcia en cada Component Principal",
        xlab="", names.arg=paste("PC",1:7),
        ylab="Percentatge d'inèrcia", ylim=c(0,35))
text(x=seq(0.7, by=1.12, length=6), y = pinerEix+1.5,
     paste0(round(pinerEix,2),"%"), cex=0.75, srt=0)
```


```{r, echo=FALSE, out.width="50%", fig.align='center', fig.cap="Barplot de la inèrcia acumulada"}
percInerAccum <- 100*cumsum(pc1$sdev[1:dim(dcon)[2]]^2)/dim(dcon)[2]
percInerAccum

# Percentatge del acumulat d'inèrcia representat en cada subespai
par(cex.main = 1, cex.lab = 1)
barplot(percInerAccum, 
        main="Percentatge d'inèrcia acumulada en cada Component Principal",
        xlab="Nombre de components", names.arg=1:7,
        ylab="Percentatge d'inèrcia acumulada",
        ylim=c(0,125))

abline(h=80, col=2, lwd=2)
text(x=seq(0.5, by=1.12, length=6), y = percInerAccum,
     paste0(round(percInerAccum,2),"%"), cex=0.85, srt=0)
```

Tenint en compte que la inèrcia equival a la proporció de la variabilitat de les dades, sabem que amb un 80% d'inèrcia, podem obtenir gairebé tota la informació. Mirant el gràfic de la inèrcia acumulada, es pot veure que amb les 4 primeres components ja obtenim el 80% de la inèrcia.
...
A continuació realitzem un gràfic de dispersió per a totes les combinacions possibles.

```{r, echo=FALSE,  out.width="60%", fig.align='center', fig.cap="Gràfica de la projecció dels individus"}
# PLOT OF INDIVIDUALS
#select your axis
eje1 <- 1
eje2 <- 2
plot(Psi[,eje1],Psi[,eje2],ylab = "Comp 2",xlab="Comp 1",main="Projecció de observacions entre PC1 y PC2")
...
a la primera component. Aquestes variables són `Nombre_ferits`, `Nombre_atacants_morts`, `Nombre_victimes` i `Nombre_atacants`. A més, aquestes dues últimes tenen un valor aproximat al 0.8 i per tant, són les que expliquen amb més precisió la primera component. Ens fixem que totes aquestes variables tenen relació amb el nombre de persones, i per tant, a la primera component li assignarem l’etiqueta de Nombre de persones involucrades. Pel que fa a l’eix vertical, només hi ha dues variables que estiguin una mica relacionades amb la segona component. Aquestes són `Valor_propietat` i `Nombre_ostatges`. 
no hi ha cap variable que estigui representada. Com que a priori, aquestes dues varaibles no tenen relació, a la segona component li posarem el nomb de la variable Valor_propieta, ja que aquesta és la que té més relació amb, ja que té un angle més petit.
...
plot(Psi[,eje1],Psi[,eje2],type="n",xlim=c(min(X,0),max(X,0)), ylim=c(-1,1), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
axis(side=1, pos= 0, labels = F)
axis(side=3, pos= 0, labels = F)
axis(side=2, pos= 0, labels = F)
axis(side=4, pos= 0, labels = F)
...
```{r, echo=FALSE, out.width="75%", fig.align='center', fig.cap="Gràfica de la projecció de la variable resposta respecte les principals components"}
#PROJECCIO DE LA VARIABLE RESPOSTA
varcat=accident$HIHAMORTS #ens demana la variable resposta (y)
plot(Psi[,1],Psi[,2],col=varcat,ylab = "Comp 2",xlab="Comp 1",main="Projecció de la variable resposta entre PC1 y PC2")
...
Gràfics de projecció de les variables categòriques

```{r,include=FALSE}
#PROJECCIO DE TOTES LES VARIABLES
#anomenem vector de categoriques
(var_categoriques <- which(sapply(bd, is.factor)))
```

Si en el mateix gràfic, hi afegim totes les categories de totes les variables categòriques, obtenim el següent:
...
Per tant, el que farem serà crear un gràfic per a cada variable categòrica, a veure si així ens és més fàcil
interpretar-los.
...
```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable regió"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
plot(Psi[,eje1],Psi[,eje2],type="n",xlim=c(-0.4,0.4), ylim=c(-0.4,0.4), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
...
bd[,dordi[1]] <- factor(bd[,dordi[1]], ordered=TRUE)
#levels(dd[,dordi[1]])
c<-1
col<-1
for(k in dordi){
  seguentColor<-colors[col]
  fdic1 = tapply(Psi[,eje1],bd[,k],mean)
  fdic2 = tapply(Psi[,eje2],bd[,k],mean) 
  ...
  text(fdic1,fdic2,labels=levels(bd[,k]),col=seguentColor, cex=0.6)
  c<-c+1
  col<-col+1
}
legend("topleft",names(bd)[dordi],pch=1,col=colors[1:length(dordi)], cex=0.6)
```

Amb la variable `regió` veiem que les cinc categories es trboen al centre del gràfic. Tot i així, veiem que la categoria `Oceania`es troba un pèl més paral·lela a la segona component, a diferència de les altres quatre, que estan més aprop de la primera component. Així doncs, encara que agafat molt amb pinces, podríem dir que la categoria `Oceania`explica millor la segona component i les altres quatre expliquen millor la primera.

**Variable Èxit_atac**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable Èxit atac"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
...
Per la variable `Èxit_atac` podem veure clarament que la categoria `No` explica la segona component, ja que es troba exactament a sobre el seu eix. Tot i així, per la categoria `Sí`, podem dir que explica d'igual manera la primera i la segona component, ja que es troba just al centre del dos eixos.

**Variable Suicidi**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable suicidi"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
...
Per la variable `Suicidi` ens trobem en una situació semblant a l'anterior. Aquí però, les dues categories expliquen millor la segona component, ja que es troben completament a sobre d'aquest eix. També podem dir que tenen una correlació semblant, és a dir, que expliquen la segona component de la mateixa manera, amb l'única diferència que la categoria `No` l'explica de forma negativa, mentre que la categoria `Sí`l'explica de forma positiva. 

**Variable Tipus_atac**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable Tipus atac"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
plot(Psi[,eje1],Psi[,eje2],type="n",xlim=c(-0.5,0.5), ylim=c(-0.5,0.5), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
#plot(X,Y,type="none",xlim=c(min(X,0),max(X,0)))
axis(side=1, pos= 0, labels = F, col="lightblue")
axis(side=3, pos= 0, labels = F, col="lightblue")
...
Per la variable `Tipus_atac`, tenim moltes categories, i la gran majoria es troben al centre de la gràfica, de manera que significa que expliquen d'igual manera la primera i segona component. Podem destacar tres categories, `Assalt (armat o no)`, `Atac a instal·lacions` i `Atac no armat`, que destaquen sobre la resta. Si ens fixem amb l'angle que formen entre les dues components, veiem que formen un angle més petit amb la primera, de forma que podem dir que aquestes tres categories expliquen millor la primera component.

**Variable Tipus_objectiu_atac**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable Tipus objectiu atac"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
...
Per la variable `Tipus_objectiu_atac` tenim encara més categories que la variable anterior, cosa que ens dificulta veure quines són les que expliquen millor una component o altra. Tot i així, podem destacar la categoria `Utilitats`, que seria la que millor explica la segona component, i les categoires `Transport`i `Desconegut`, que serien les que millor expliquen la primera component.

**Variable Nacionalitat_objectiu**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable Nacionalitat objectiu"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
...
Per la variable `Nacionalitat_objectiu` veiem com la gran majoria de categories expliquen millor la primera component, i aproximadament de la mateixa mesura. Tot i així, la categoria `desconegut` i `oceania`expliquen millor la segona, ja que es troben just o gairebé just a sobre l'eix de les ordenades.

**Variable Atac_reclamat**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable Atac reclamat"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
plot(Psi[,eje1],Psi[,eje2],type="n",xlim=c(-0.2,0.2), ylim=c(-0.2,0.2), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
#plot(X,Y,type="none",xlim=c(min(X,0),max(X,0)))
axis(side=1, pos= 0, labels = F, col="lightblue")
axis(side=3, pos= 0, labels = F, col="lightblue")
...
Per la variable `Atac_reclamat` veiem que les tres categories es troben just al centre del gràfic, tot i així, gràcies a que hem apliat el gràfic, podem dir que la categoria `Desconegut` explica millor la segona component, mentre que les categories `No`i `Sí` expliquen millor la primera component.

**Variable Tipus_arma**

```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Gràfica de la projecció de la variable Tipus arma"}
#scale the projected variables
#X<-fm*U[,eje1]
#Y<-fm*U[,eje2]
#represent numerical variables in background
...
Finalment, per la variable `Tipus_arma`, veiem que destaquen les categories `Incendi` i `Descongut`. Totes dues formen un angle semblant entre la primer i la segona componet, de forma que podem dir que les expliquen d'igual manera. Pel que fa a les altres tres categories, es troben al centre de la gràfica i per tant no podem saber quina de les dues components expliquen millor.
