...
```{r}
varcat=accident$HIHAMORTS 
colores <- function(varcat){
# la función rainbow() devuelve un vector que contiene el número de colores distintos
col <- rainbow(length(unique(varcat)))
return(col[as.numeric(as.factor(varcat))])
}

#par(mfrow = c(1,2))
# Observaciones sobre PC1 y PC2
plot(pc1$x[,1:2], col = colores(accident$HIHAMORTS ), 
     pch = 19, 
     xlab = "PC1", 
     ylab = "PC2")
legend("topright", levels(accident$HIHAMORTS), pch = 19, col = colores(levels(accident$HIHAMORTS)))
# Observaciones sobre PC1 y PC3
#plot(pc1$x[,c(1, 3)], col = colores(accident$HIHAMORTS ), 
 #    pch = 19, 
 #    xlab = "Z1", 
 #    ylab = "Z3")
```
...
```{r}
fviz_contrib(pc1, choice = "var", axes = 1)
```
...
#Y<-fm*U[,eje2]
#represent numerical variables in background
plot(Psi[,1],Psi[,2],type="n",xlim=c(-1,1), ylim=c(-1,1), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
#plot(X,Y,type="none",xlim=c(min(X,0),max(X,0)))
axis(side=1, pos= 0, labels = F, col="lightblue")
axis(side=3, pos= 0, labels = F, col="lightblue")
...
#Y<-fm*U[,eje2]
#represent numerical variables in background
plot(Psi[,1],Psi[,2],type="n",xlim=c(-0.5,0.5), ylim=c(-0.5,0.5), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
#plot(X,Y,type="none",xlim=c(min(X,0),max(X,0)))
axis(side=1, pos= 0, labels = F, col="lightblue")
axis(side=3, pos= 0, labels = F, col="lightblue")
...
#Y<-fm*U[,eje2]
#represent numerical variables in background
plot(Psi[,1],Psi[,2],type="n",xlim=c(min(X,0),max(X,0)), ylim=c(min(Y,0),max(Y,0)), 
     xlab = "CP1: Nombre de persones involucrades", ylab = "CP2: Valor de la propietat")
#plot(X,Y,type="none",xlim=c(min(X,0),max(X,0)))
axis(side=1, pos= 0, labels = F, col="lightblue")
axis(side=3, pos= 0, labels = F, col="lightblue")
